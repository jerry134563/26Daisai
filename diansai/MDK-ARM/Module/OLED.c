...
void OLED_W_SCL(uint8_t BitValue)
{
	...
	if(BitValue)
	{
		HAL_GPIO_WritePin(OLED_IIC_SCL_GPIO_PORT, OLED_IIC_SCL_GPIO_PIN, GPIO_PIN_SET);
	}
	else
	{
		HAL_GPIO_WritePin(OLED_IIC_SCL_GPIO_PORT, OLED_IIC_SCL_GPIO_PIN, GPIO_PIN_RESET);
	}
	delay_us(2);
...
void OLED_W_SDA(uint8_t BitValue)
{
	...
	if(BitValue)
	{
		HAL_GPIO_WritePin(OLED_IIC_SDA_GPIO_PORT, OLED_IIC_SDA_GPIO_PIN, GPIO_PIN_SET);
	}
	else
	{
		HAL_GPIO_WritePin(OLED_IIC_SDA_GPIO_PORT, OLED_IIC_SDA_GPIO_PIN, GPIO_PIN_RESET);
	}
	delay_us(2);
	/*如果单片机速度过快，可在此添加适量延时，以避免超出I2C通信的最大速度*/
}
...
{	
	/*在初始化前，加入适量延时，待OLED供电稳定,这边用delay函数延时，h7跑这个循环初始化跑不了会跳出OLED的初始化函数，估计是自适应能力*/
	uint32_t i, j;	
	for (i = 0; i < 1000; i ++)
	{
		for (j = 0; j < 1000; j ++);
