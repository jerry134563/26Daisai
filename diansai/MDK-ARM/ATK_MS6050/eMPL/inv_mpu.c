...
//#define get_ms(p)      do {*p = HAL_GetTick();} while (0)   /* 获取毫秒级时间戳函数 */ 
...
{
	uint8_t result;
	result = mpu_init(NULL);                                /* 硬件初始化 */
	if(result != 0)
	{
		return 1;
	}
    result = mpu_set_sensors(INV_XYZ_GYRO | INV_XYZ_ACCEL);       /* 开启指定传感器 */
	if(result != 0)
	{
		return 2;
	}
	...
	result = dmp_load_motion_driver_firmware();                   /* 加载DMP镜像 */
	if(result != 0)
	{
		return 5;
	}
    result = dmp_set_orientation(                                 /* 设置陀螺仪方向 */
                                inv_orientation_matrix_to_scalar(gyro_orientation));
	if(result != 0)
	{
		return 6;
	}
    result = dmp_enable_feature(  DMP_FEATURE_6X_LP_QUAT      |   /* 设置DMP功能 */
                                DMP_FEATURE_TAP             |
    ...
    result = dmp_set_fifo_rate(DEFAULT_MPU_HZ);                   /* 设置DMP输出速率 */
	if(result != 0)
	{
		return 8;
	}
	...
	result = atk_ms6050_run_self_test();                          /* 传感器自测试 */
	if(result != 0)
	{
		return 10;
	}
